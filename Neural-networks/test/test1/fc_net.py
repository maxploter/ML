...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        ############################################################################
        hidden, cache1 = affine_arctan_forward(X, self.params['W1'], self.params['b1'])
        ...
        ############################################################################
        probs = scores # sigmoid(scores)

        data_loss = binary_cross_entropy_loss(probs, y)

        # Add regularization to the loss
        reg_loss = 0.5 * self.reg * sum(np.sum(self.params[f'W{i}'] ** 2) for i in range(1, self.num_layers + 1))
        loss = data_loss + reg_loss

        dscores = (probs - y) / (y.shape[0] * probs * (1 - probs))
        dscores_with_softmax = (probs - y) / y.shape[0]
        # print(dscores)
        # print(dscores_with_softmax)

        # Backprop into the second layer
        dhidden, dW2, db2 = affine_backward(dscores, cache2)
        dW2 += self.reg * self.params[f'W{self.num_layers}']
        grads[f'W{self.num_layers}'] = dW2
        grads[f'b{self.num_layers}'] = db2

        # Backprop into the first layer
        _, dW1, db1 = affine_arctan_backward(dhidden, cache1)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for hidden_dim in hidden_dims:
            self.params[f'W{i}'] = np.random.randn(n, hidden_dim) * weight_scale
            self.params[f'b{i}'] = np.zeros(hidden_dim)
            n = hidden_dim
            i += 1

        self.params[f'W{i}'] = np.random.randn(n, num_classes) * weight_scale
    ...
    def loss(self, X, y=None):
        ...
        for layer in range(1, self.num_layers):
            hidden, cache = affine_relu_forward(x, self.params[f'W{layer}'], self.params[f'b{layer}'])
        ...
        correct_logprobs = -np.log(softmax[range(N), y])
        ...
        dX, dW, db = affine_backward(dscores, cache_history.pop())
        ...
        for layer in range(self.num_layers - 1, 0, -1):
            # if self.use_dropout:
                # dX = dropout_backward(dX, dropout_cache.pop())
...
def binary_cross_entropy_loss(probs, y):
    N = y.shape[0]
    loss = -np.sum(y * np.log(probs) + (1 - y) * np.log(1 - probs)) / N
    return loss